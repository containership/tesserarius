...
module.exports = {
    ...
    add_rule(chain, rule, callback) {
        ...
        const schema = {
            ...
            comment: Joi.string(),
            destination_interface: Joi.string(),
        ...
        Joi.validate(rule, schema, (err) => {
            ...
            if(rule.destination_interface) {
                args.push(constants.FLAGS.DESTINATION_INTERFACE);
                args.push(rule.destination_interface);
            }
...
function execute_command(args, callback) {
    ...
    proc.on('close', (code) => {
        if(code !== 0 || !_.isEmpty(error)) {
            const error_message = _.isEmpty(error) ? `Process returned exit code: ${code}` : error.join(' ');
